...
if LoadNetParameters
    ...
    NetParameters.ActivateModelDiscovery = 'No';                                  %Accepted values: 'Yes', 'No'
    ...
    NetParameters.ModelDiscoverySmoothing = 'SGMixed3';                                  %Accepted values: 'TV', 'TVMixed', 'TVMixed2', 'SG', 'SGMixed', 'SGMixed2' 'No'
    ...
    NetParameters.ActivateWhitenessMask = 'No';                                    %Accepted values: 'Yes', 'No'
    NetParameters.WhitenessLagCounter = 1;
    NetParameters.WhitenessIterationCheck = 5;
    NetParameters.WhitenessUpdateCheck = 20;
    NetParameters.WhitenessDecreaseThreshold = -1e-2;
    ...
    NetParameters.BackPropagation = 'Truncated';                                     %Accepted values: 'Truncated', 'Complete'
    ...
    NetParameters.GainLearningRate = (5e-2)/TrainingBatchSize;
    NetParameters.GainLearningRateReduction = 1;
    NetParameters.DynamicsLearningRate = (2e-2)/TrainingBatchSize;
    NetParameters.DynamicsLearningRateReduction = 0.6;
    ...
    NetParameters.Penalty0 = 0;%1e0;                          %Penalty for (1/2)*||States{Layers+1}-StateTrue||^2
    ...
    NetParameters.Penalty3 = 1e1/(NetParameters.StateDimension*NetParameters.ObservationDimension); %Penalty for (1/2)*||L*TensorizedGains||^2
    ...
    NetParameters.WeightEpsilon = 1e-16;
...
for TrainingBatchInd = 2:TrainingBatchNum
    ...
    for BatchInd = 1:TrainingBatchSize 
        ...
        %Check whiteness
        if  strcmp(NetParameters.ActivateWhitenessMask,'Yes')&&( (TrainingBatchInd > NetParameters.WhitenessIterationCheck)&&(AdamInd > NetParameters.WhitenessUpdateCheck) )
        ...
        UpdateModelDiscovery = strcmp(NetParameters.ActivateModelDiscovery,'Yes')&&( ( AdamInd > NetParameters.ModelDiscoveryForceCheck )||( ~any(sum(LaggedGainMask,2)>0) ) );
        ...
        if UpdateModelDiscovery
            ...
            TempSparseMat = UpdateSparseMat(NetWeights,States,Dynamic,NetParameters);
            TempHiddenDynamics = NetWeights{end}{Dynamic};
            NetWeights = InitializeWeights(NetParameters);
            NetWeights{end}{end} = TempSparseMat;
            NetWeights{end}{Dynamic} = TempHiddenDynamics;

            %Reset moments since the governing model has changed
    ...
    semilogy(PeriodogramResidues(1:TrainingBatchInd,NetParameters.ObservationDimension+1:end));
    legend(strcat('ObservedState:',num2str( (1:NetParameters.ObservationDimension)' )),'Location','southwest');
